...
$sql = "SELECT id, task_title, task_completed_percent, created_at, due_date 
        FROM school_tasks 
        WHERE school_id = ?";
...
while($row = $result->fetch_assoc()){
    ...
    $members_html = '<ul class="list-unstyled order-list m-b-0 m-b-0">';
    $count = 0;
    $total_members = 0;
    ...
    while($m = $members_result->fetch_assoc()){
        ...
        if ($count < 3) {
            ...
            $members_html .= '<li class="team-member team-member-sm">
                                <img class="rounded-circle" src="uploads/profile/'.$img.'" alt="'.$name.'" data-toggle="tooltip" title="'.$name.'">
                              </li>';
            $count++;
        }
    }
