...
if ($action === 'insert') {
    ...
    $stmt = $conn->prepare("INSERT INTO meeting_requests (school_id, requested_by, requester_id, with_meeting, id_meeter, title, agenda, created_at) VALUES (?, 'teacher', ?, ?, ?, ?, ?, NOW())");
...
// Fetch all meetings
elseif ($action === 'fetch') {
    $result = $conn->query("SELECT id, with_meeting, id_meeter, title,status, agenda FROM meeting_requests WHERE school_id='$school_id' AND status!='approved' ORDER BY id DESC");
